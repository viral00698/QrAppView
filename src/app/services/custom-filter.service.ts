...
export class CustomFilterService {
  ...
  groupByDay(data: any) {
    ...
    for (const item of data) {
      ...
      const orderTime = new Date(item[1]).getTime();

      if (orderTime >= startOfDay && orderTime <= endOfDay) {
        const date = new Date(orderTime);
        const hour24 = date.getHours(); // 0 to 23

        const hour12 = hour24 % 12 || 12; // Convert to 12-hour format
        const ampm = hour24 >= 12 ? 'PM' : 'AM';
        const label = `${hour12} ${ampm}`; // e.g., "1 PM"

        const currentCount = dayMap.get(label) || 0;
        dayMap.set(label, currentCount + 1);
      }
    }
  ...
  groupBylast90Days(data: any) {
    ...
    const thirtyDaysAgoIST = new Date(todayIST.getTime());
    thirtyDaysAgoIST.setDate(todayIST.getDate() - 89); // Includes today
  ...
  groupBylast180Days(data: any) {
    ...
    const thirtyDaysAgoIST = new Date(todayIST.getTime());
    thirtyDaysAgoIST.setDate(todayIST.getDate() - 179); // Includes today
  ...
  filterOrders(type: 'daily' | 'last7Days' | 'monthly' | 'yearly' | 'hourly' | 'last30Days' | 'last90Days' | 'last180Days', orderStats: any) {
    switch (type) {
      case 'daily':
      ...
      case 'last180Days':
        return this.groupBylast180Days(orderStats)
